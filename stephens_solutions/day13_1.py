...
class Layer(object):
    ...
    def tick(self):
        ...
        if self.pos == self.depth and self.velocity > 0:
            self.velocity = -self.velocity
        elif self.pos == 1 and self.velocity < 0:
            self.velocity = -self.velocity


SEVERITY = 0
LAYERS = {int(line.split(": ")[0]): Layer(
    *tuple(line.split(": "))) for line in fileinput.input()}
for i in xrange(max(LAYERS) + 1):
    if i in LAYERS and LAYERS[i].pos == 1:
