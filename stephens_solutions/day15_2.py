...
def generator(value, factor, mul_check):
    while True:
        next_value = (value * factor) % 2147483647
        if next_value % mul_check == 0:
            yield next_value
        value = next_value
...
cmp_gen = cmp_gens(lowbits(generator(512, 16807, 4), 16),
                   lowbits(generator(191, 48271, 8), 16))
