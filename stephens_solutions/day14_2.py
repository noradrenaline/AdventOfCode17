...
from gmpy2 import popcount  # pylint: disable=E0611
...
class KnotHash(String):
    ...
    def getbin(self):
        return bin(int(self.dense(), 16))[2:].zfill(128)
...
def proc_image(image):
    ...
    for pixel in xrange(width * height):
        ...
        if image[y][x] == '1':
            g.add_edge(pixel, pixel)
            if x > 0 and image[y][x - 1] == '1':
                g.add_edge(pixel, pixel - 1)
            if y > 0 and image[y - 1][x] == '1':
                g.add_edge(pixel, pixel - width)
    return len(list(nx.components.connected_components(g)))

print 'generating image...'
image = [KnotHash('{}-{}'.format(sys.argv[1], i)).getbin() for i in xrange(128)]
print "processing image..."
print proc_image(image)
